# generated by datamodel-codegen:
#   filename:  openapi_cleaned.json
#   timestamp: 2026-08-02T19:56:26+00:00

from __future__ import annotations

from enum import Enum
from typing import Any

from pydantic import BaseModel, Field, confloat


class Base64File(BaseModel):
    mimetype: str
    data: str


class QRCodeValue(BaseModel):
    value: str


class RequestCodeRequest(BaseModel):
    phoneNumber: str = Field(
        ...,
        description="Mobile phone number in international format",
        examples=["12132132130"],
    )
    method: str | None = Field(
        None,
        description="How would you like to receive the one time code for registration? |sms|voice. Leave empty for Web pairing.",
        examples=[None],
    )


class PasskeyAllowedCredential(BaseModel):
    id: str = Field(..., description="Base64url-encoded credential ID.")
    type: str = Field(..., description='Always "public-key".', examples=["public-key"])
    transports: list[str] | None = Field(
        None,
        description="Authenticator transports the credential supports.",
        examples=[["internal", "hybrid"]],
    )


class PasskeyChallenge(BaseModel):
    challenge: str = Field(..., description="Base64url-encoded challenge to sign.")
    timeout: float = Field(
        ...,
        description="How long the challenge is valid for, in milliseconds.",
        examples=[60000],
    )
    rpId: str = Field(..., description="Relying party ID.", examples=["web.whatsapp.com"])
    allowCredentials: list[PasskeyAllowedCredential]
    userVerification: str = Field(..., examples=["required"])
    extensions: dict[str, Any] | None = Field(None, description="WebAuthn extensions requested by WhatsApp.")


class PasskeyAssertionResponseData(BaseModel):
    clientDataJSON: str = Field(..., description="Base64url-encoded clientDataJSON from the authenticator.")
    authenticatorData: str = Field(..., description="Base64url-encoded authenticatorData from the authenticator.")
    signature: str = Field(..., description="Base64url-encoded signature from the authenticator.")
    userHandle: str | None = Field(
        None,
        description="Base64url-encoded user handle, if returned by the authenticator.",
    )


class PasskeyAssertionRequest(BaseModel):
    id: str = Field(
        ...,
        description="Credential ID, as returned by navigator.credentials.get().toJSON().",
    )
    rawId: str = Field(..., description="Base64url-encoded raw credential ID.")
    type: str = Field(..., description='Always "public-key".', examples=["public-key"])
    response: PasskeyAssertionResponseData


class PasskeyConfirmationResponse(BaseModel):
    code: str = Field(
        ...,
        description="The code the user must verify against the one shown on their phone.",
        examples=["1234"],
    )


class SessionActionsDTO(BaseModel):
    read: bool | None = Field(None, description="Read session data (messages, contacts, chats, groups, etc.)")
    send: bool | None = Field(
        None,
        description="Send messages and manage session entities (groups, labels, channels, contacts, profile)",
    )
    control: bool | None = Field(
        None,
        description="Session lifecycle: start, stop, restart, logout, authenticate",
    )
    setting: bool | None = Field(None, description="Session config: update session settings")
    app: bool | None = Field(None, description="Manage apps")
    delete: bool | None = Field(None, description="Delete the session")


class ApiKeyRequest(BaseModel):
    isAdmin: bool = Field(..., examples=[False])
    session: str = Field(..., examples=["default"])
    isActive: bool = Field(..., examples=[True])
    actions: SessionActionsDTO | None = None


class ApiKeyDTO(BaseModel):
    id: str = Field(..., examples=["key_id_00000000000000000000000000"])
    key: str = Field(..., examples=["key_11111111111AAAAAAAAAAAAAAAAAAAAA"])
    isActive: bool = Field(..., examples=[True])
    isAdmin: bool = Field(..., examples=[False])
    session: str | None = Field(None, examples=["default"])
    actions: SessionActionsDTO | None = None


class ScopedApiKeyRequest(BaseModel):
    session: str = Field(..., examples=["default"])


class ChatWootCommandsConfig(BaseModel):
    server: bool
    queue: bool | None = False


class Sort(Enum):
    activity_newest = "activity_newest"
    created_newest = "created_newest"
    created_oldest = "created_oldest"
    activity_oldest = "activity_oldest"


class StatusEnum(Enum):
    open = "open"
    pending = "pending"
    snoozed = "snoozed"
    resolved = "resolved"


class ChatWootConversationsConfig(BaseModel):
    markAsRead: bool | None = Field(
        True,
        description="Process message.ack events to mark ChatWoot conversations as read. Enabled by default.",
    )
    sort: Sort
    status: list[StatusEnum]


class LinkPreview(Enum):
    OFF = "OFF"
    LG = "LG"
    HG = "HG"


class ChatWootAppConfig(BaseModel):
    url: str
    accountId: float
    accountToken: str
    inboxId: float
    inboxIdentifier: str
    linkPreview: LinkPreview | None = "OFF"
    locale: str
    templates: dict[str, Any] | None = None
    commands: ChatWootCommandsConfig | None = None
    conversations: ChatWootConversationsConfig | None = None


class CallsAppChannelConfig(BaseModel):
    reject: bool = Field(..., description="Reject incoming calls for this chat type")
    message: str | None = Field(
        None,
        description="Optional auto-reply message sent after the call is rejected. If empty, no message is sent.",
    )
    waitBeforeDecline: confloat(ge=0.0) | None = Field(
        None,
        description="Seconds to wait before declining the call. If not set or undefined, the call is declined immediately (0 seconds).",
    )
    waitBeforeResponse: confloat(ge=0.0) | None = Field(
        None,
        description="Seconds to wait before sending the auto-reply message. If not set or undefined, the message is sent immediately (0 seconds).",
    )


class CallsAppConfig(BaseModel):
    dm: CallsAppChannelConfig = Field(..., description="Rules applied to direct messages (non-group calls)")
    group: CallsAppChannelConfig = Field(..., description="Rules applied to group calls")


class McpAppConfig(BaseModel):
    actions: SessionActionsDTO = Field(..., description="Permission scopes for the generated API key.")
    key_id: str | None = Field(
        None,
        description="ID of the API key created for this app. Read-only.",
        examples=["key_id_00000000000000000000000000"],
    )
    key: str | None = Field(
        None,
        description="The API key value. Populated on read; not persisted in this record.",
        examples=["key_11111111111AAAAAAAAAAAAAAAAAAAAA"],
    )


class App1(Enum):
    chatwoot = "chatwoot"
    calls = "calls"
    mcp = "mcp"


class App(BaseModel):
    enabled: bool | None = Field(
        True,
        description="Enable or disable this app without deleting it. If omitted, treated as enabled (true).",
    )
    id: str
    session: str
    app: App1
    config: dict[str, Any]


class EnforcementType(Enum):
    DEFAULT = "DEFAULT"
    BIZ_QUALITY = "BIZ_QUALITY"
    BIZ_COMMERCE_VIOLATION_ADULT = "BIZ_COMMERCE_VIOLATION_ADULT"
    BIZ_COMMERCE_VIOLATION_ALCOHOL = "BIZ_COMMERCE_VIOLATION_ALCOHOL"
    BIZ_COMMERCE_VIOLATION_ANIMALS = "BIZ_COMMERCE_VIOLATION_ANIMALS"
    BIZ_COMMERCE_VIOLATION_BODY_PARTS_FLUIDS = "BIZ_COMMERCE_VIOLATION_BODY_PARTS_FLUIDS"
    BIZ_COMMERCE_VIOLATION_DATING = "BIZ_COMMERCE_VIOLATION_DATING"
    BIZ_COMMERCE_VIOLATION_DIGITAL_SERVICES_PRODUCTS = "BIZ_COMMERCE_VIOLATION_DIGITAL_SERVICES_PRODUCTS"
    BIZ_COMMERCE_VIOLATION_DRUGS = "BIZ_COMMERCE_VIOLATION_DRUGS"
    BIZ_COMMERCE_VIOLATION_DRUGS_ONLY_OTC = "BIZ_COMMERCE_VIOLATION_DRUGS_ONLY_OTC"
    BIZ_COMMERCE_VIOLATION_GAMBLING = "BIZ_COMMERCE_VIOLATION_GAMBLING"
    BIZ_COMMERCE_VIOLATION_HEALTHCARE = "BIZ_COMMERCE_VIOLATION_HEALTHCARE"
    BIZ_COMMERCE_VIOLATION_REAL_FAKE_CURRENCY = "BIZ_COMMERCE_VIOLATION_REAL_FAKE_CURRENCY"
    BIZ_COMMERCE_VIOLATION_SUPPLEMENTS = "BIZ_COMMERCE_VIOLATION_SUPPLEMENTS"
    BIZ_COMMERCE_VIOLATION_TOBACCO = "BIZ_COMMERCE_VIOLATION_TOBACCO"
    BIZ_COMMERCE_VIOLATION_VIOLENT_CONTENT = "BIZ_COMMERCE_VIOLATION_VIOLENT_CONTENT"
    BIZ_COMMERCE_VIOLATION_WEAPONS = "BIZ_COMMERCE_VIOLATION_WEAPONS"
    WEB_COMPANION_ONLY = "WEB_COMPANION_ONLY"
    RESTRICT_ALL_COMPANIONS = "RESTRICT_ALL_COMPANIONS"


class ReachoutTimelockData(BaseModel):
    enforcementType: EnforcementType = Field(
        ...,
        description="Raw WhatsApp enforcement type. Informational only - it does not change what is blocked. WhatsApp may introduce new values, so treat it as an open set.",
        examples=["RESTRICT_ALL_COMPANIONS"],
    )
    isActive: bool = Field(..., examples=[True])
    timeEnforcementEnds: float = Field(
        ...,
        description="Unix timestamp (seconds) when the enforcement ends.",
        examples=[1784477333],
    )


class MeInfo(BaseModel):
    id: str = Field(..., examples=["11111111111@c.us"])
    lid: str | None = Field(None, examples=["123123@lid"])
    jid: str | None = Field(
        None,
        description="Your id with device number",
        examples=["123123:123@s.whatsapp.net"],
    )
    reachoutTimelock: ReachoutTimelockData | None = Field(
        None,
        description="WhatsApp reachout timelock (account restriction) info. Null if no enforcement has been seen for the account.",
    )
    pushName: str


class ProxyConfig(BaseModel):
    server: str = Field(..., examples=["localhost:3128"])
    username: str | None = Field(None, examples=[None])
    password: str | None = Field(None, examples=[None])


class IgnoreConfig(BaseModel):
    status: bool | None = Field(None, description="Ignore a status@broadcast (stories) events")
    groups: bool | None = Field(None, description="Ignore groups events")
    channels: bool | None = Field(None, description="Ignore channels events")
    broadcast: bool | None = Field(None, description="Ignore broadcast events (broadcast list and status)")


class ClientSessionConfig(BaseModel):
    deviceName: str | None = None
    browserName: str | None = None


class NowebStoreConfig(BaseModel):
    enabled: bool = Field(
        ...,
        description="Enable or disable the store for contacts, chats, and messages.",
        examples=[True],
    )
    fullSync: bool = Field(
        ...,
        description="Enable full sync on session initialization (when scanning QR code).\nFull sync will download all contacts, chats, and messages from the phone.\nIf disabled, only messages early than 90 days will be downloaded and some contacts may be missing.",
    )


class NowebConfig(BaseModel):
    markOnline: bool = Field(..., description="Mark the session as online when it connects to the server.")
    store: NowebStoreConfig | None = None


class GowsStorageConfig(BaseModel):
    messages: bool | None = Field(
        None,
        description="Store messages locally. Set to false to disable; omit or null to keep enabled.",
        examples=[True],
    )
    groups: bool | None = Field(
        None,
        description="Store groups locally. Set to false to disable; omit or null to keep enabled.",
        examples=[True],
    )
    chats: bool | None = Field(
        None,
        description="Store chats locally. Set to false to disable; omit or null to keep enabled.",
        examples=[True],
    )
    labels: bool | None = Field(
        None,
        description="Store labels locally. Set to false to disable; omit or null to keep enabled.",
        examples=[True],
    )


class GowsConfig(BaseModel):
    storage: GowsStorageConfig | None = None


class WebjsConfig(BaseModel):
    tagsEventsOn: bool | None = Field(
        False,
        description="Enable emission of special 'tag:*' engine events required for presence.update and message.ack.\nWARNING: Enabling this may have performance and stability impact. Disabled by default.",
    )


class HmacConfiguration(BaseModel):
    key: str | None = Field(None, examples=["your-secret-key"])


class Policy(Enum):
    linear = "linear"
    exponential = "exponential"
    constant = "constant"


class RetriesConfiguration(BaseModel):
    delaySeconds: float | None = Field(None, examples=[2])
    attempts: float | None = Field(None, examples=[15])
    policy: Policy | None = Field(None, examples=["linear"])


class CustomHeader(BaseModel):
    name: str = Field(..., examples=["X-My-Custom-Header"])
    value: str = Field(..., examples=["Value"])


class WebhookConfig(BaseModel):
    url: str = Field(
        ...,
        description="You can use https://docs.webhook.site/ to test webhooks and see the payload",
        examples=["https://webhook.site/11111111-1111-1111-1111-11111111"],
    )
    events: list[dict[str, Any]] = Field(..., examples=[["message", "session.status"]])
    hmac: HmacConfiguration | None = Field(None, examples=[None])
    retries: RetriesConfiguration | None = Field(None, examples=[None])
    customHeaders: list[CustomHeader] | None = Field(None, examples=[None])


class SessionConfig(BaseModel):
    metadata: dict[str, Any] | None = Field(
        None,
        description="Metadata for the session. You'll get 'metadata' in all webhooks.",
        examples=[{"user.id": "123", "user.email": "email@example.com"}],
    )
    proxy: ProxyConfig | None = Field(None, examples=[None])
    debug: bool | None = False
    ignore: IgnoreConfig | None = Field(
        None,
        description="Ignore some events related to specific chats",
        examples=[{"status": None, "groups": None, "channels": None}],
    )
    client: ClientSessionConfig | None = Field(
        None,
        description="How connected session renders in device - in format 'Browser (Device)' - Firefox (MacOS)",
        examples=[{"browserName": "Firefox", "deviceName": "MacOS"}],
    )
    noweb: NowebConfig | None = Field(None, examples=[{"store": {"enabled": True, "fullSync": False}}])
    gows: GowsConfig | None = Field(
        None,
        examples=[
            {
                "storage": {
                    "messages": True,
                    "groups": True,
                    "chats": True,
                    "labels": True,
                }
            }
        ],
    )
    webjs: WebjsConfig | None = Field(None, description="WebJS-specific settings.")
    webhooks: list[WebhookConfig] | None = None


class Timestamps(BaseModel):
    activity: float


class Status(Enum):
    STOPPED = "STOPPED"
    STARTING = "STARTING"
    SCAN_QR_CODE = "SCAN_QR_CODE"
    PASSKEY_REQUIRED = "PASSKEY_REQUIRED"
    PASSKEY_CONFIRMATION_REQUIRED = "PASSKEY_CONFIRMATION_REQUIRED"
    WORKING = "WORKING"
    FAILED = "FAILED"


class SessionInfo(BaseModel):
    name: str = Field(..., description="Session name (id)", examples=["default"])
    apps: list[App] | None = Field(None, description="Apps configured for the session.")
    me: MeInfo | None = None
    assignedWorker: str | None = None
    presence: dict[str, Any]
    timestamps: Timestamps
    status: Status
    config: SessionConfig | None = None


class SessionCreateRequest(BaseModel):
    name: str | None = Field(None, description="Session name (id)", examples=["default"])
    apps: list[App] | None = Field(None, description="Apps to be synchronized for this session.")
    start: bool | None = Field(True, description="Start session after creation", examples=[True])
    config: SessionConfig | None = None


class SessionDTO(BaseModel):
    name: str = Field(..., description="Session name (id)", examples=["default"])
    status: Status
    config: SessionConfig | None = None


class SessionUpdateRequest(BaseModel):
    apps: list[App] | None = Field(None, description="Apps to be synchronized for this session.")
    config: SessionConfig | None = None


class SessionStartDeprecatedRequest(BaseModel):
    name: str = Field(..., description="Session name (id)", examples=["default"])
    config: SessionConfig | None = None


class SessionStopDeprecatedRequest(BaseModel):
    name: str = Field(..., description="Session name (id)", examples=["default"])
    logout: bool | None = Field(False, description="Stop and logout from the session.", examples=[False])


class SessionLogoutDeprecatedRequest(BaseModel):
    name: str = Field(..., description="Session name (id)", examples=["default"])


class MyProfile(BaseModel):
    id: str = Field(..., examples=["11111111111@c.us"])
    picture: str = Field(..., examples=["https://example.com/picture.jpg"])
    name: str


class ProfileNameRequest(BaseModel):
    name: str = Field(..., examples=["My New Name"])


class Result(BaseModel):
    success: bool


class ProfileStatusRequest(BaseModel):
    status: str = Field(..., examples=["🎉 Hey there! I am using WhatsApp 🎉"])


class RemoteFile(BaseModel):
    mimetype: str = Field(..., description="MIME type of the attachment.", examples=["image/jpeg"])
    filename: str | None = Field(
        None,
        description="Document file name. Value can be null",
        examples=["filename.jpg"],
    )
    url: str = Field(..., examples=["https://picsum.photos/1024"])


class BinaryFile(BaseModel):
    mimetype: str = Field(..., description="MIME type of the attachment.", examples=["image/jpeg"])
    filename: str | None = Field(None, description="Document file name. Optional", examples=["filename.jpeg"])
    data: str = Field(
        ...,
        description="Base64-encoded data of the file",
        examples=[
            ""
        ],
    )


class ProfilePictureRequest(BaseModel):
    file: RemoteFile | BinaryFile


class MessageTextRequest(BaseModel):
    chatId: str = Field(..., examples=["11111111111@c.us"])
    id: str | None = Field(None, description="Pre-generated message id", examples=["BBBBBBBBBBBBBBBBB"])
    reply_to: str | None = Field(
        None,
        description="The ID of the message to reply to - false_11111111111@c.us_AAAAAAAAAAAAAAAAAAAA",
        examples=[None],
    )
    text: str
    linkPreview: bool | None = True
    linkPreviewHighQuality: bool | None = False
    session: str


class S3MediaData(BaseModel):
    Bucket: str = Field(..., description="The name of the S3 bucket", examples=["my-bucket"])
    Key: str = Field(
        ...,
        description="The key of the object in the S3 bucket",
        examples=["default/false_11111111111@c.us_AAAAAAAAAAAAAAAAAAAA.oga"],
    )


class WAMedia(BaseModel):
    url: str | None = Field(
        None,
        description="The URL for the media in the message if any",
        examples=["http://localhost:3000/api/files/false_11111111111@c.us_AAAAAAAAAAAAAAAAAAAA.oga"],
    )
    mimetype: str | None = Field(
        None,
        description="mimetype for the media in the message if any",
        examples=["audio/jpeg"],
    )
    filename: str | None = Field(
        None,
        description="The original filename in mediaUrl in the message if any",
        examples=["example.pdf"],
    )
    s3: S3MediaData | None = Field(
        None,
        description="S3 attributes for the media in the message if you are using S3 media storage",
    )
    error: dict[str, Any] | None = Field(
        None,
        description="Error message if there's an error downloading the media",
        examples=[None],
    )


class WALocation(BaseModel):
    latitude: str
    longitude: str
    live: bool
    name: str | None = None
    address: str | None = None
    url: str | None = None
    description: str | None = None
    thumbnail: str | None = None


class ReplyToMessage(BaseModel):
    id: str = Field(..., description="Message ID", examples=["AAAAAAAAAAAAAAAAAAAA"])
    participant: str | None = Field(None, examples=["11111111111@c.us"])
    body: str | None = Field(None, examples=["Hello!"])
    hasMedia: bool = Field(..., description="Indicates if the message has media available for download")
    media: WAMedia | None = Field(None, description="Media object for the message if any and downloaded")
    field_data: dict[str, Any] | None = Field(None, alias="_data", description="Raw data from reply's message")


class Source(Enum):
    api = "api"
    app = "app"


class Ack(Enum):
    number__1 = -1
    number_0 = 0
    number_1 = 1
    number_2 = 2
    number_3 = 3
    number_4 = 4


class WAMessage(BaseModel):
    id: str = Field(
        ...,
        description="Message ID",
        examples=["false_11111111111@c.us_AAAAAAAAAAAAAAAAAAAA"],
    )
    timestamp: float = Field(
        ...,
        description="Unix timestamp for when the message was created",
        examples=[1666943582],
    )
    from_: str = Field(
        ...,
        alias="from",
        description="ID for the Chat that this message was sent to, except if the message was sent by the current user ",
        examples=["11111111111@c.us"],
    )
    fromMe: bool = Field(..., description="Indicates if the message was sent by the current user")
    source: Source = Field(
        ...,
        description='The device that sent the message - either API or APP. Available in events (webhooks/websockets) only and only "fromMe: true" messages.',
        examples=["api"],
    )
    to: str = Field(
        ...,
        description="\n* ID for who this message is for.\n* If the message is sent by the current user, it will be the Chat to which the message is being sent.\n* If the message is sent by another user, it will be the ID for the current user.\n",
        examples=["11111111111@c.us"],
    )
    participant: str = Field(..., description="For groups - participant who sent the message")
    body: str = Field(..., description="Message content")
    hasMedia: bool = Field(..., description="Indicates if the message has media available for download")
    media: WAMedia | None = Field(None, description="Media object for the message if any and downloaded")
    mediaUrl: str = Field(
        ...,
        deprecated=True,
        description="Use `media.url` instead! The URL for the media in the message if any",
        examples=["http://localhost:3000/api/files/false_11111111111@c.us_AAAAAAAAAAAAAAAAAAAA.oga"],
    )
    ack: Ack = Field(..., description="ACK status for the message")
    ackName: str = Field(..., description="ACK status name for the message")
    author: str | None = Field(
        None,
        description="If the message was sent to a group, this field will contain the user that sent the message.",
    )
    location: WALocation | None = Field(
        None,
        description='Location information contained in the message, if the message is type "location"',
    )
    vCards: list[str] | None = Field(None, description="List of vCards contained in the message.")
    field_data: dict[str, Any] | None = Field(
        None,
        alias="_data",
        description="Message in a raw format that we get from WhatsApp. May be changed anytime, use it with caution! It depends a lot on the underlying backend.",
    )
    replyTo: ReplyToMessage | None = None


class MessageImageRequest(BaseModel):
    chatId: str = Field(..., examples=["11111111111@c.us"])
    file: RemoteFile | BinaryFile
    reply_to: str | None = Field(
        None,
        description="The ID of the message to reply to - false_11111111111@c.us_AAAAAAAAAAAAAAAAAAAA",
        examples=[None],
    )
    caption: str | None = None
    session: str


class MessageFileRequest(BaseModel):
    chatId: str = Field(..., examples=["11111111111@c.us"])
    file: RemoteFile | BinaryFile
    reply_to: str | None = Field(
        None,
        description="The ID of the message to reply to - false_11111111111@c.us_AAAAAAAAAAAAAAAAAAAA",
        examples=[None],
    )
    caption: str | None = None
    session: str


class VoiceBinaryFile(BaseModel):
    mimetype: dict[str, Any] = Field(..., description="MIME type of the attachment.")
    filename: dict[str, Any] = Field(..., description="Document file name. Optional")
    data: str = Field(
        ...,
        description="Base64-encoded data of the file",
        examples=[
            ""
        ],
    )


class VoiceRemoteFile(BaseModel):
    mimetype: dict[str, Any] = Field(..., description="MIME type of the attachment.")
    url: str = Field(
        ...,
        examples=["https://github.com/devlikeapro/waha/raw/core/examples/dev.likeapro.opus"],
    )


class MessageVoiceRequest(BaseModel):
    chatId: str = Field(..., examples=["11111111111@c.us"])
    file: VoiceRemoteFile | VoiceBinaryFile
    reply_to: str | None = Field(
        None,
        description="The ID of the message to reply to - false_11111111111@c.us_AAAAAAAAAAAAAAAAAAAA",
        examples=[None],
    )
    convert: bool = Field(
        ...,
        description="Convert the input file to the required format using ffmpeg before sending",
        examples=[True],
    )
    session: str


class VideoRemoteFile(BaseModel):
    mimetype: dict[str, Any] = Field(..., description="MIME type of the attachment.")
    filename: dict[str, Any] = Field(..., description="Document file name. Optional")
    url: str = Field(
        ...,
        examples=["https://github.com/devlikeapro/waha/raw/core/examples/video.mp4"],
    )


class VideoBinaryFile(BaseModel):
    mimetype: dict[str, Any] = Field(..., description="MIME type of the attachment.")
    filename: dict[str, Any] = Field(..., description="Document file name. Optional")
    data: str = Field(
        ...,
        description="Base64-encoded data of the file",
        examples=[
            ""
        ],
    )


class MessageVideoRequest(BaseModel):
    chatId: str = Field(..., examples=["11111111111@c.us"])
    file: VideoRemoteFile | VideoBinaryFile
    reply_to: str | None = Field(
        None,
        description="The ID of the message to reply to - false_11111111111@c.us_AAAAAAAAAAAAAAAAAAAA",
        examples=[None],
    )
    asNote: bool | None = Field(
        None,
        description="Send as video note (aka instant or round video).",
        examples=[False],
    )
    convert: bool = Field(
        ...,
        description="Convert the input file to the required format using ffmpeg before sending",
        examples=[True],
    )
    caption: str | None = "Just watch at this!"
    session: str


class FileURL(BaseModel):
    url: str | None = Field(None, examples=["https://picsum.photos/1024"])


class FileContent(BaseModel):
    data: str | None = Field(
        None,
        description="Base64-encoded data of the file",
        examples=[
            ""
        ],
    )


class LinkPreviewData(BaseModel):
    image: FileURL | FileContent | None = Field(None, examples=[{"url": "https://picsum.photos/1024"}])
    url: str
    title: str
    description: str


class MessageLinkCustomPreviewRequest(BaseModel):
    chatId: str = Field(..., examples=["11111111111@c.us"])
    text: str = Field(
        ...,
        description="The text to send. MUST include the URL provided in preview.url",
    )
    reply_to: str | None = Field(
        None,
        description="The ID of the message to reply to - false_11111111111@c.us_AAAAAAAAAAAAAAAAAAAA",
        examples=[None],
    )
    linkPreviewHighQuality: bool | None = True
    preview: LinkPreviewData
    session: str


class Type(Enum):
    reply = "reply"
    url = "url"
    call = "call"
    copy = "copy"


class Button(BaseModel):
    text: str = Field(..., examples=["Button Text"])
    id: str | None = Field(None, examples=["321321"])
    url: str | None = Field(None, examples=["https://example.com"])
    phoneNumber: str | None = Field(None, examples=["+1234567890"])
    copyCode: str | None = Field(None, examples=["4321"])
    type: Type


class SendButtonsRequest(BaseModel):
    chatId: str = Field(..., examples=["11111111111@c.us"])
    header: str = Field(..., examples=["How are you?"])
    headerImage: RemoteFile | BinaryFile | None = None
    body: str = Field(..., examples=["Tell us how are you please 🙏"])
    footer: str = Field(..., examples=["If you have any questions, please send it in the chat"])
    buttons: list[Button] = Field(
        ...,
        examples=[
            [
                {"type": "reply", "text": "I am good!"},
                {"type": "call", "text": "Call us", "phoneNumber": "+1234567890"},
                {"type": "copy", "text": "Copy code", "copyCode": "4321"},
                {
                    "type": "url",
                    "text": "How did you do that?",
                    "url": "https://waha.devlike.pro",
                },
            ]
        ],
    )
    session: str


class Row(BaseModel):
    title: str = Field(..., examples=["Option 1"])
    description: str | None = Field(None, examples=["Description of option 1"])
    rowId: str = Field(..., examples=["option1"])


class Section(BaseModel):
    title: str = Field(..., examples=["Menu"])
    rows: list[Row] = Field(
        ...,
        examples=[
            [
                {
                    "title": "Option 1",
                    "rowId": "option1",
                    "description": "First option",
                },
                {
                    "title": "Option 2",
                    "rowId": "option2",
                    "description": "Second option",
                },
            ]
        ],
    )


class SendListMessage(BaseModel):
    title: str = Field(..., examples=["Example List"])
    description: str | None = Field(None, examples=["Choose one of the options"])
    footer: str | None = Field(None, examples=["Footer note"])
    button: str = Field(..., examples=["Select"])
    sections: list[Section] = Field(
        ...,
        examples=[
            [
                {
                    "title": "Section 1",
                    "rows": [
                        {
                            "title": "Option 1",
                            "rowId": "option1",
                            "description": "Description 1",
                        },
                        {
                            "title": "Option 2",
                            "rowId": "option2",
                            "description": "Description 2",
                        },
                    ],
                }
            ]
        ],
    )


class SendListRequest(BaseModel):
    chatId: str = Field(..., examples=["11111111111@c.us"])
    message: SendListMessage = Field(
        ...,
        examples=[
            {
                "title": "Simple Menu",
                "description": "Please choose an option",
                "footer": "Thank you!",
                "button": "Choose",
                "sections": [
                    {
                        "title": "Main",
                        "rows": [
                            {
                                "title": "Option 1",
                                "rowId": "option1",
                                "description": None,
                            },
                            {
                                "title": "Option 2",
                                "rowId": "option2",
                                "description": None,
                            },
                            {
                                "title": "Option 3",
                                "rowId": "option3",
                                "description": None,
                            },
                        ],
                    }
                ],
            }
        ],
    )
    reply_to: str | None = Field(
        None,
        description="The ID of the message to reply to - false_11111111111@c.us_AAAAAAAAAAAAAAAAAAAA",
        examples=[None],
    )
    session: str


class MessageForwardRequest(BaseModel):
    chatId: str = Field(..., examples=["11111111111@c.us"])
    id: str | None = Field(None, description="Pre-generated message id", examples=["BBBBBBBBBBBBBBBBB"])
    messageId: str = Field(..., examples=["false_11111111111@c.us_AAAAAAAAAAAAAAAAAAAA"])
    session: str


class SendSeenRequest(BaseModel):
    chatId: str = Field(..., examples=["11111111111@c.us"])
    messageId: str | None = Field(None, deprecated=True, examples=[None])
    messageIds: list[str] | None = Field(None, examples=[["false_11111111111@c.us_AAAAAAAAAAAAAAAAAAAA"]])
    participant: str | None = Field(
        None,
        description="NOWEB engine only - the ID of the user that sent the message (undefined for individual chats)",
        examples=["11111111111@c.us"],
    )
    session: str


class ChatRequest(BaseModel):
    chatId: str = Field(..., examples=["11111111111@c.us"])
    session: str


class MessageReactionRequest(BaseModel):
    messageId: str = Field(..., examples=["false_11111111111@c.us_AAAAAAAAAAAAAAAAAAAA"])
    reaction: str = Field(
        ...,
        description="Emoji to react with. Send an empty string to remove the reaction",
        examples=["👍"],
    )
    session: str


class MessageStarRequest(BaseModel):
    messageId: str = Field(..., examples=["false_11111111111@c.us_AAAAAAAAAAAAAAAAAAAA"])
    chatId: str = Field(..., examples=["11111111111@c.us"])
    star: bool
    session: str


class MessagePoll(BaseModel):
    name: str = Field(..., examples=["How are you?"])
    options: list[str] = Field(..., examples=[["Awesome!", "Good!", "Not bad!"]])
    multipleAnswers: dict[str, Any]


class MessagePollRequest(BaseModel):
    chatId: str = Field(..., examples=["11111111111@c.us"])
    id: str | None = Field(None, description="Pre-generated message id", examples=["BBBBBBBBBBBBBBBBB"])
    reply_to: str | None = Field(
        None,
        description="The ID of the message to reply to - false_11111111111@c.us_AAAAAAAAAAAAAAAAAAAA",
        examples=[None],
    )
    poll: MessagePoll
    session: str


class MessagePollVoteRequest(BaseModel):
    chatId: str = Field(..., examples=["11111111111@c.us"])
    pollMessageId: str = Field(
        ...,
        description="The ID of the poll message. Format: {fromMe}_{chatID}_{messageId}[_{participant}] or just ID for GOWS",
        examples=["false_11111111111@c.us_AAAAAAAAAAAAAAAAAAAA"],
    )
    pollServerId: float | None = Field(
        None,
        description="Only for Channels - server message id (if known); if omitted, API may look it up in the storage",
        examples=[None],
    )
    votes: list[list[str]]
    session: str


class MessageLocationRequest(BaseModel):
    chatId: str = Field(..., examples=["11111111111@c.us"])
    id: str | None = Field(None, description="Pre-generated message id", examples=["BBBBBBBBBBBBBBBBB"])
    latitude: float = Field(..., examples=[38.8937255])
    longitude: float = Field(..., examples=[-77.0969763])
    title: str = Field(..., examples=["Our office"])
    reply_to: str | None = Field(
        None,
        description="The ID of the message to reply to - false_11111111111@c.us_AAAAAAAAAAAAAAAAAAAA",
        examples=[None],
    )
    session: str


class Contact(BaseModel):
    fullName: str = Field(..., description="The full name of the contact", examples=["John Doe"])
    organization: str | None = Field(None, description="The organization of the contact", examples=["Company Name"])
    phoneNumber: str = Field(..., description="The phone number of the contact", examples=["+91 11111 11111"])
    whatsappId: str | None = Field(
        None,
        description="The whatsapp id of the contact. DO NOT add + or @c.us",
        examples=["911111111111"],
    )
    vcard: str


class VCardContact(BaseModel):
    vcard: str = Field(
        ...,
        description="The vcard string",
        examples=[
            "BEGIN:VCARD\nVERSION:3.0\nFN:Jane Doe\nORG:Company Name;\nTEL;type=CELL;type=VOICE;waid=911111111111:+91 11111 11111\nEND:VCARD"
        ],
    )


class MessageContactVcardRequest(BaseModel):
    chatId: str = Field(..., examples=["11111111111@c.us"])
    id: str | None = Field(None, description="Pre-generated message id", examples=["BBBBBBBBBBBBBBBBB"])
    contacts: list[VCardContact | Contact]
    reply_to: str | None = Field(
        None,
        description="The ID of the message to reply to - false_11111111111@c.us_AAAAAAAAAAAAAAAAAAAA",
        examples=[None],
    )
    session: str


class MessageButtonReply(BaseModel):
    chatId: str = Field(..., examples=["11111111111@c.us"])
    replyTo: str | None = Field(
        None,
        description="The ID of the message to reply to - false_11111111111@c.us_AAAAAAAAAAAAAAAAAAAA",
        examples=[None],
    )
    selectedDisplayText: str
    selectedButtonID: str
    session: str


class WANumberExistResult(BaseModel):
    chatId: str | None = Field(
        None,
        examples=["Chat id for the phone number. Undefined if the number does not exist"],
    )
    numberExists: bool


class MessageReplyRequest(BaseModel):
    chatId: str = Field(..., examples=["11111111111@c.us"])
    id: str | None = Field(None, description="Pre-generated message id", examples=["BBBBBBBBBBBBBBBBB"])
    reply_to: str | None = Field(
        None,
        description="The ID of the message to reply to - false_11111111111@c.us_AAAAAAAAAAAAAAAAAAAA",
        examples=[None],
    )
    text: str
    linkPreview: bool | None = True
    linkPreviewHighQuality: bool | None = False
    session: str


class MessageLinkPreviewRequest(BaseModel):
    chatId: str = Field(..., examples=["11111111111@c.us"])
    id: str | None = Field(None, description="Pre-generated message id", examples=["BBBBBBBBBBBBBBBBB"])
    url: str
    title: str
    session: str


class NewMessageIDResponse(BaseModel):
    id: str = Field(..., description="Pre-generated message id", examples=["BBBBBBBBBBBBBBBBB"])


class ChatSummary(BaseModel):
    id: str
    name: str
    picture: str
    lastMessage: dict[str, Any]
    field_chat: dict[str, Any] = Field(..., alias="_chat")


class GetChatsOverviewParams(BaseModel):
    merge: bool | None = Field(
        True,
        description="Merge LID (@lid) and phone-number (@c.us) chats referencing the same contact",
        examples=[True],
    )
    limit: float | None = 20
    offset: float | None = None


class OverviewFilter(BaseModel):
    ids: list[str] | None = Field(None, description="Filter by chat ids", examples=[["111111111@c.us"]])


class OverviewBodyRequest(BaseModel):
    pagination: GetChatsOverviewParams
    filter: OverviewFilter | None = None


class ChatPictureResponse(BaseModel):
    url: str


class ReadChatMessagesResponse(BaseModel):
    ids: list[str] | None = Field(None, description="Messages IDs that have been read")


class PinMessageRequest(BaseModel):
    duration: float = Field(
        ...,
        description="Duration in seconds. 24 hours (86400), 7 days (604800), 30 days (2592000)",
        examples=[86400],
    )


class EditMessageRequest(BaseModel):
    text: str
    linkPreview: bool | None = True
    linkPreviewHighQuality: bool | None = False


class RejectCallRequest(BaseModel):
    from_: str = Field(..., alias="from", examples=["11111111111@c.us"])
    id: str = Field(..., description="Call ID", examples=["ABCDEFGABCDEFGABCDEFGABCDEFG"])


class Role(Enum):
    OWNER = "OWNER"
    ADMIN = "ADMIN"
    SUBSCRIBER = "SUBSCRIBER"
    GUEST = "GUEST"


class Channel(BaseModel):
    id: str = Field(..., description="Newsletter id", examples=["123123123123@newsletter"])
    name: str = Field(..., description="Channel name", examples=["Channel Name"])
    invite: str = Field(
        ...,
        description="Invite link",
        examples=["https://www.whatsapp.com/channel/111111111111111111111111"],
    )
    preview: str | None = Field(
        None,
        description="Preview for channel's picture",
        examples=["https://mmg.whatsapp.net/m1/v/t24/An&_nc_cat=10"],
    )
    picture: str | None = Field(
        None,
        description="Channel's picture",
        examples=["https://mmg.whatsapp.net/m1/v/t24/An&_nc_cat=10"],
    )
    role: Role
    description: str | None = None
    verified: bool
    subscribersCount: float


class CreateChannelRequest(BaseModel):
    name: str = Field(..., examples=["Channel Name"])
    description: str | None = Field(None, examples=["Channel Description"])
    picture: RemoteFile | BinaryFile | None = None


class ChannelMessage(BaseModel):
    reactions: dict[str, float] = Field(..., examples=[{"👍": 10, "❤️": 5}])
    message: WAMessage
    viewCount: float


class ChannelSearchByView(BaseModel):
    view: str
    countries: list[str]
    categories: list[str]
    limit: float
    startCursor: str


class ChannelPagination(BaseModel):
    startCursor: str
    endCursor: str
    hasNextPage: bool
    hasPreviousPage: bool


class ChannelPublicInfo(BaseModel):
    id: str = Field(..., description="Newsletter id", examples=["123123123123@newsletter"])
    name: str = Field(..., description="Channel name", examples=["Channel Name"])
    invite: str = Field(
        ...,
        description="Invite link",
        examples=["https://www.whatsapp.com/channel/111111111111111111111111"],
    )
    preview: str | None = Field(
        None,
        description="Preview for channel's picture",
        examples=["https://mmg.whatsapp.net/m1/v/t24/An&_nc_cat=10"],
    )
    picture: str | None = Field(
        None,
        description="Channel's picture",
        examples=["https://mmg.whatsapp.net/m1/v/t24/An&_nc_cat=10"],
    )
    description: str | None = None
    verified: bool
    subscribersCount: float


class ChannelListResult(BaseModel):
    page: ChannelPagination
    channels: list[ChannelPublicInfo]


class ChannelSearchByText(BaseModel):
    text: str
    categories: list[str]
    limit: float
    startCursor: str


class ChannelView(BaseModel):
    value: str
    name: str


class ChannelCountry(BaseModel):
    code: str
    name: str


class ChannelCategory(BaseModel):
    value: str
    name: str


class TextStatus(BaseModel):
    id: str | None = Field(
        None,
        description="Pre-generated status message id",
        examples=["BBBBBBBBBBBBBBBBB"],
    )
    contacts: list[str] | None = Field(None, description="Contact list to send the status to.", examples=[None])
    text: str
    backgroundColor: str
    font: float
    linkPreview: bool | None = True
    linkPreviewHighQuality: bool | None = False


class ImageStatus(BaseModel):
    id: str | None = Field(
        None,
        description="Pre-generated status message id",
        examples=["BBBBBBBBBBBBBBBBB"],
    )
    contacts: list[str] | None = Field(None, description="Contact list to send the status to.", examples=[None])
    file: RemoteFile | BinaryFile
    caption: str | None = None


class VoiceStatus(BaseModel):
    id: str | None = Field(
        None,
        description="Pre-generated status message id",
        examples=["BBBBBBBBBBBBBBBBB"],
    )
    contacts: list[str] | None = Field(None, description="Contact list to send the status to.", examples=[None])
    file: VoiceRemoteFile | VoiceBinaryFile
    convert: bool = Field(
        ...,
        description="Convert the input file to the required format using ffmpeg before sending",
        examples=[True],
    )
    backgroundColor: str


class VideoStatus(BaseModel):
    id: str | None = Field(
        None,
        description="Pre-generated status message id",
        examples=["BBBBBBBBBBBBBBBBB"],
    )
    contacts: list[str] | None = Field(None, description="Contact list to send the status to.", examples=[None])
    file: VideoRemoteFile | VideoBinaryFile
    convert: bool = Field(
        ...,
        description="Convert the input file to the required format using ffmpeg before sending",
        examples=[True],
    )
    caption: str | None = None


class DeleteStatusRequest(BaseModel):
    id: str | None = Field(None, description="Status message id to delete", examples=["AAAAAAAAAAAAAAAAA"])
    contacts: list[str] | None = Field(None, description="Contact list to send the status to.", examples=[None])


class Label(BaseModel):
    id: str = Field(..., description="Label ID", examples=["1"])
    name: str = Field(..., description="Label name", examples=["Lead"])
    color: float = Field(..., description="Color number, not hex", examples=[0])
    colorHex: str = Field(..., description="Color in hex", examples=["#ff9485"])


class LabelBody(BaseModel):
    name: str = Field(..., description="Label name", examples=["Lead"])
    colorHex: str | None = Field(None, description="Color in hex", examples=["#ff9485"])
    color: float | None = Field(None, description="Color number, not hex", examples=[None])


class LabelID(BaseModel):
    id: str = Field(..., description="Label ID", examples=["1"])


class SetLabelsRequest(BaseModel):
    labels: list[LabelID]


class ContactRequest(BaseModel):
    contactId: str = Field(..., examples=["11111111111@c.us"])
    session: str


class ContactUpdateBody(BaseModel):
    firstName: str = Field(..., description="Contact First Name", examples=["John"])
    lastName: str = Field(..., description="Contact Last Name", examples=["Doe"])


class LidToPhoneNumber(BaseModel):
    lid: str | None = Field(None, description="Linked ID for the user", examples=["1111111@lid"])
    pn: str | None = Field(
        None,
        description="Phone number (chat id) for the user",
        examples=["3333333@c.us"],
    )


class CountResponse(BaseModel):
    count: float


class Participant(BaseModel):
    id: str = Field(..., examples=["123456789@c.us"])


class CreateGroupRequest(BaseModel):
    name: str
    participants: list[Participant]


class JoinGroupRequest(BaseModel):
    code: str = Field(
        ...,
        description="Group code (123) or url (https://chat.whatsapp.com/123)",
        examples=["https://chat.whatsapp.com/1234567890abcdef"],
    )


class JoinGroupResponse(BaseModel):
    id: str = Field(..., description="Group ID", examples=["123@g.us"])


class DescriptionRequest(BaseModel):
    description: str


class SubjectRequest(BaseModel):
    subject: str


class SettingsSecurityChangeInfo(BaseModel):
    adminsOnly: bool


class SettingsMemberAddMode(BaseModel):
    membersCanAddNewMember: bool


class Role1(Enum):
    left = "left"
    participant = "participant"
    admin = "admin"
    superadmin = "superadmin"


class GroupParticipant(BaseModel):
    id: str = Field(..., description="Member ID in @c.us or @lid format", examples=["123456789@lid"])
    pn: str | None = Field(None, description="Member ID in @c.us format", examples=["123456789@c.us"])
    role: Role1 = Field(..., examples=["participant"])


class ParticipantsRequest(BaseModel):
    participants: list[Participant]


class Presence(Enum):
    offline = "offline"
    online = "online"
    typing = "typing"
    recording = "recording"
    paused = "paused"


class WAHASessionPresence(BaseModel):
    chatId: str | None = Field(
        None,
        description="Chat ID - either group id or contact id. Required for chat-related presence statuses; omit for ONLINE/OFFLINE.",
        examples=["11111111111@c.us"],
    )
    presence: Presence


class LastKnownPresence(Enum):
    offline = "offline"
    online = "online"
    typing = "typing"
    recording = "recording"
    paused = "paused"


class WAHAPresenceData(BaseModel):
    participant: str = Field(
        ...,
        description="Chat ID - participant or contact id",
        examples=["11111111111@c.us"],
    )
    lastSeen: float | None = Field(None, examples=[1686568773])
    lastKnownPresence: LastKnownPresence


class WAHAChatPresences(BaseModel):
    id: str = Field(
        ...,
        description="Chat ID - either group id or contact id",
        examples=["11111111111@c.us"],
    )
    presences: list[WAHAPresenceData]


class EventLocation(BaseModel):
    name: str = Field(..., description="Name of the location", examples=["Luxe Nail Studio 💅"])


class EventMessage(BaseModel):
    name: str = Field(..., description="Name of the event", examples=["John's Nail Appointment 💅"])
    description: str | None = Field(
        None,
        description="Description of the event",
        examples=[
            "It's time for your nail care session! 🌟\\n\\nYou'll be getting a *classic gel manicure* – clean, polished, and long-lasting. 💖\\n\\n📍 *Location:* Luxe Nail Studio\\nWe're on the *2nd floor of the Plaza Mall*, next to the flower shop. Look for the *pink neon sign*!\\n\\nFeel free to arrive *5–10 mins early* so we can get started on time 😊"
        ],
    )
    startTime: float = Field(
        ...,
        description="Start time of the event (Unix timestamp in seconds)",
        examples=[2063137000],
    )
    endTime: float | None = Field(
        None,
        description="End time of the event (Unix timestamp in seconds)",
        examples=[None],
    )
    location: EventLocation | None = Field(None, description="Location of the event")
    extraGuestsAllowed: bool | None = Field(None, description="Whether extra guests are allowed", examples=[False])


class EventMessageRequest(BaseModel):
    chatId: str = Field(..., examples=["11111111111@c.us"])
    reply_to: str | None = Field(
        None,
        description="The ID of the message to reply to - false_11111111111@c.us_AAAAAAAAAAAAAAAAAAAA",
        examples=[None],
    )
    event: EventMessage


class PingResponse(BaseModel):
    message: str


class Worker(BaseModel):
    id: str


class WAHAEnvironment(BaseModel):
    version: str = Field(..., examples=["YYYY.MM.BUILD"])
    engine: str = Field(..., examples=["WEBJS"])
    tier: str = Field(..., examples=["PLUS"])
    browser: str = Field(..., examples=["/usr/path/to/bin/google-chrome"])
    platform: str = Field(..., examples=["linux/x86"])
    worker: Worker


class WorkerInfo(BaseModel):
    id: str = Field(..., description="The worker ID.", examples=["waha"])


class ServerStatusResponse(BaseModel):
    startTimestamp: float = Field(
        ...,
        description="The timestamp when the server started (milliseconds).",
        examples=[1723788847247],
    )
    uptime: float = Field(..., description="The uptime of the server in milliseconds.", examples=[3600000])
    worker: WorkerInfo


class StopRequest(BaseModel):
    force: bool | None = Field(
        False,
        description="By default, it gracefully stops the server, but you can force it to terminate immediately.",
        examples=[False],
    )


class StopResponse(BaseModel):
    stopping: bool = Field(..., description="Always 'true' if the server is stopping.", examples=[True])


class VoiceFileDTO(BaseModel):
    url: str | None = Field(
        None,
        description="The URL for the voice file",
        examples=["https://github.com/devlikeapro/waha/raw/core/examples/voice.mp3"],
    )
    data: str | None = Field(None, description="Base64 content of the file", examples=[None])


class VideoFileDTO(BaseModel):
    url: str | None = Field(
        None,
        description="The URL for the video file",
        examples=["https://github.com/devlikeapro/waha/raw/core/examples/video.mp4"],
    )
    data: str | None = Field(None, description="Base64 content of the file", examples=[None])


class SessionStatusPoint(BaseModel):
    status: Status
    timestamp: float


class WASessionStatusBody(BaseModel):
    name: str = Field(..., examples=["default"])
    data: dict[str, Any] | None = Field(
        None,
        description="Extra info that belongs to the current status, null for most of them.\nPASSKEY_REQUIRED - the WebAuthn challenge, pass it to navigator.credentials.get({ publicKey: data }).\nPASSKEY_CONFIRMATION_REQUIRED - { code } - the code to verify against the phone.\nWORKING - { reachoutTimelock } - present when WhatsApp restricts the account from messaging new contacts (the cause of 463 errors on send); the session stays connected and the restriction lifts automatically.",
        examples=[None],
    )
    status: Status
    statuses: list[SessionStatusPoint]


class Engine(Enum):
    WEBJS = "WEBJS"
    WPP = "WPP"
    NOWEB = "NOWEB"
    GOWS = "GOWS"


class WAHAWebhookSessionStatus(BaseModel):
    id: str = Field(
        ...,
        description="Unique identifier for the event - lower case ULID format. https://github.com/ulid/spec",
        examples=["evt_01aaaaaaaaaaaaaaaaaaaaaaaa"],
    )
    timestamp: float = Field(
        ...,
        description="Unix timestamp (ms) for when the event was created.",
        examples=[1634567890123],
    )
    session: str = Field(..., examples=["default"])
    metadata: dict[str, Any] | None = Field(
        None,
        description="Metadata for the session.",
        examples=[{"user.id": "123", "user.email": "email@example.com"}],
    )
    engine: Engine = Field(..., examples=["WEBJS"])
    event: dict[str, Any] = Field(..., description="The event is triggered when the session status changes.")
    payload: WASessionStatusBody
    me: MeInfo | None = None
    environment: WAHAEnvironment


class WAHAWebhookMessage(BaseModel):
    id: str = Field(
        ...,
        description="Unique identifier for the event - lower case ULID format. https://github.com/ulid/spec",
        examples=["evt_01aaaaaaaaaaaaaaaaaaaaaaaa"],
    )
    timestamp: float = Field(
        ...,
        description="Unix timestamp (ms) for when the event was created.",
        examples=[1634567890123],
    )
    session: str = Field(..., examples=["default"])
    metadata: dict[str, Any] | None = Field(
        None,
        description="Metadata for the session.",
        examples=[{"user.id": "123", "user.email": "email@example.com"}],
    )
    engine: Engine = Field(..., examples=["WEBJS"])
    event: dict[str, Any] = Field(..., description="Incoming message.")
    payload: WAMessage
    me: MeInfo | None = None
    environment: WAHAEnvironment


class WAReaction(BaseModel):
    text: str = Field(
        ...,
        description="Reaction to the message. Either the reaction (emoji) or empty string to remove the reaction",
    )
    messageId: str = Field(
        ...,
        description="Message ID for the message to react to",
        examples=["false_11111111111@c.us_AAAAAAAAAAAAAAAAAAAA"],
    )


class WAMessageReaction(BaseModel):
    id: str = Field(
        ...,
        description="Message ID",
        examples=["false_11111111111@c.us_AAAAAAAAAAAAAAAAAAAA"],
    )
    timestamp: float = Field(
        ...,
        description="Unix timestamp for when the message was created",
        examples=[1666943582],
    )
    from_: str = Field(
        ...,
        alias="from",
        description="ID for the Chat that this message was sent to, except if the message was sent by the current user ",
        examples=["11111111111@c.us"],
    )
    fromMe: bool = Field(..., description="Indicates if the message was sent by the current user")
    source: Source = Field(
        ...,
        description='The device that sent the message - either API or APP. Available in events (webhooks/websockets) only and only "fromMe: true" messages.',
        examples=["api"],
    )
    to: str = Field(
        ...,
        description="\n* ID for who this message is for.\n* If the message is sent by the current user, it will be the Chat to which the message is being sent.\n* If the message is sent by another user, it will be the ID for the current user.\n",
        examples=["11111111111@c.us"],
    )
    participant: str = Field(..., description="For groups - participant who sent the message")
    reaction: WAReaction = Field(
        ...,
        description="Reaction to the message. Either the reaction (emoji) or empty string to remove the reaction",
    )


class WAHAWebhookMessageReaction(BaseModel):
    id: str = Field(
        ...,
        description="Unique identifier for the event - lower case ULID format. https://github.com/ulid/spec",
        examples=["evt_01aaaaaaaaaaaaaaaaaaaaaaaa"],
    )
    timestamp: float = Field(
        ...,
        description="Unix timestamp (ms) for when the event was created.",
        examples=[1634567890123],
    )
    session: str = Field(..., examples=["default"])
    metadata: dict[str, Any] | None = Field(
        None,
        description="Metadata for the session.",
        examples=[{"user.id": "123", "user.email": "email@example.com"}],
    )
    engine: Engine = Field(..., examples=["WEBJS"])
    event: dict[str, Any] = Field(
        ...,
        description="The event is triggered when a user reacts or removes a reaction.",
    )
    payload: WAMessageReaction
    me: MeInfo | None = None
    environment: WAHAEnvironment


class WAHAWebhookMessageAny(BaseModel):
    id: str = Field(
        ...,
        description="Unique identifier for the event - lower case ULID format. https://github.com/ulid/spec",
        examples=["evt_01aaaaaaaaaaaaaaaaaaaaaaaa"],
    )
    timestamp: float = Field(
        ...,
        description="Unix timestamp (ms) for when the event was created.",
        examples=[1634567890123],
    )
    session: str = Field(..., examples=["default"])
    metadata: dict[str, Any] | None = Field(
        None,
        description="Metadata for the session.",
        examples=[{"user.id": "123", "user.email": "email@example.com"}],
    )
    engine: Engine = Field(..., examples=["WEBJS"])
    event: dict[str, Any] = Field(..., description="Fired on all message creations, including your own.")
    payload: WAMessage
    me: MeInfo | None = None
    environment: WAHAEnvironment


class WAMessageAckBody(BaseModel):
    id: str = Field(
        ...,
        description="Message ID",
        examples=["false_11111111111@c.us_AAAAAAAAAAAAAAAAAAAA"],
    )
    from_: str = Field(..., alias="from", examples=["11111111111@c.us"])
    to: str = Field(..., examples=["11111111111@c.us"])
    participant: str = Field(..., examples=["11111111111@c.us"])
    fromMe: bool
    ack: Ack
    ackName: str
    field_data: dict[str, Any] | None = Field(None, alias="_data")


class WAHAWebhookMessageAck(BaseModel):
    id: str = Field(
        ...,
        description="Unique identifier for the event - lower case ULID format. https://github.com/ulid/spec",
        examples=["evt_01aaaaaaaaaaaaaaaaaaaaaaaa"],
    )
    timestamp: float = Field(
        ...,
        description="Unix timestamp (ms) for when the event was created.",
        examples=[1634567890123],
    )
    session: str = Field(..., examples=["default"])
    metadata: dict[str, Any] | None = Field(
        None,
        description="Metadata for the session.",
        examples=[{"user.id": "123", "user.email": "email@example.com"}],
    )
    engine: Engine = Field(..., examples=["WEBJS"])
    event: dict[str, Any] = Field(
        ...,
        description="Receive events when server or recipient gets the message, read or played it (contacts only).",
    )
    payload: WAMessageAckBody
    me: MeInfo | None = None
    environment: WAHAEnvironment


class WAHAWebhookMessageAckGroup(BaseModel):
    id: str = Field(
        ...,
        description="Unique identifier for the event - lower case ULID format. https://github.com/ulid/spec",
        examples=["evt_01aaaaaaaaaaaaaaaaaaaaaaaa"],
    )
    timestamp: float = Field(
        ...,
        description="Unix timestamp (ms) for when the event was created.",
        examples=[1634567890123],
    )
    session: str = Field(..., examples=["default"])
    metadata: dict[str, Any] | None = Field(
        None,
        description="Metadata for the session.",
        examples=[{"user.id": "123", "user.email": "email@example.com"}],
    )
    engine: Engine = Field(..., examples=["WEBJS"])
    event: dict[str, Any] = Field(
        ...,
        description="Receive events when participants in a group read or play messages.",
    )
    payload: WAMessageAckBody
    me: MeInfo | None = None
    environment: WAHAEnvironment


class WAMessageRevokedBody(BaseModel):
    revokedMessageId: str | None = Field(
        None,
        description="ID of the message that was revoked",
        examples=["A06CA7BB5DD8C8F705628CDB7E3A33C9"],
    )
    after: WAMessage | None = None
    before: WAMessage | None = None
    field_data: dict[str, Any] | None = Field(None, alias="_data")


class WAHAWebhookMessageRevoked(BaseModel):
    id: str = Field(
        ...,
        description="Unique identifier for the event - lower case ULID format. https://github.com/ulid/spec",
        examples=["evt_01aaaaaaaaaaaaaaaaaaaaaaaa"],
    )
    timestamp: float = Field(
        ...,
        description="Unix timestamp (ms) for when the event was created.",
        examples=[1634567890123],
    )
    session: str = Field(..., examples=["default"])
    metadata: dict[str, Any] | None = Field(
        None,
        description="Metadata for the session.",
        examples=[{"user.id": "123", "user.email": "email@example.com"}],
    )
    engine: Engine = Field(..., examples=["WEBJS"])
    event: dict[str, Any] = Field(
        ...,
        description="The event is triggered when a user, whether it be you or any other participant, revokes a previously sent message.",
    )
    payload: WAMessageRevokedBody
    me: MeInfo | None = None
    environment: WAHAEnvironment


class WAMessageEditedBody(BaseModel):
    id: str = Field(
        ...,
        description="Message ID",
        examples=["false_11111111111@c.us_AAAAAAAAAAAAAAAAAAAA"],
    )
    timestamp: float = Field(
        ...,
        description="Unix timestamp for when the message was created",
        examples=[1666943582],
    )
    from_: str = Field(
        ...,
        alias="from",
        description="ID for the Chat that this message was sent to, except if the message was sent by the current user ",
        examples=["11111111111@c.us"],
    )
    fromMe: bool = Field(..., description="Indicates if the message was sent by the current user")
    source: Source = Field(
        ...,
        description='The device that sent the message - either API or APP. Available in events (webhooks/websockets) only and only "fromMe: true" messages.',
        examples=["api"],
    )
    to: str = Field(
        ...,
        description="\n* ID for who this message is for.\n* If the message is sent by the current user, it will be the Chat to which the message is being sent.\n* If the message is sent by another user, it will be the ID for the current user.\n",
        examples=["11111111111@c.us"],
    )
    participant: str = Field(..., description="For groups - participant who sent the message")
    body: str = Field(..., description="Message content")
    hasMedia: bool = Field(..., description="Indicates if the message has media available for download")
    media: WAMedia | None = Field(None, description="Media object for the message if any and downloaded")
    mediaUrl: str = Field(
        ...,
        deprecated=True,
        description="Use `media.url` instead! The URL for the media in the message if any",
        examples=["http://localhost:3000/api/files/false_11111111111@c.us_AAAAAAAAAAAAAAAAAAAA.oga"],
    )
    ack: Ack = Field(..., description="ACK status for the message")
    ackName: str = Field(..., description="ACK status name for the message")
    author: str | None = Field(
        None,
        description="If the message was sent to a group, this field will contain the user that sent the message.",
    )
    location: WALocation | None = Field(
        None,
        description='Location information contained in the message, if the message is type "location"',
    )
    vCards: list[str] | None = Field(None, description="List of vCards contained in the message.")
    field_data: dict[str, Any] | None = Field(
        None,
        alias="_data",
        description="Message in a raw format that we get from WhatsApp. May be changed anytime, use it with caution! It depends a lot on the underlying backend.",
    )
    editedMessageId: str | None = Field(
        None,
        description="ID of the original message that was edited",
        examples=["A06CA7BB5DD8C8F705628CDB7E3A33C9"],
    )
    replyTo: ReplyToMessage | None = None


class WAHAWebhookMessageEdited(BaseModel):
    id: str = Field(
        ...,
        description="Unique identifier for the event - lower case ULID format. https://github.com/ulid/spec",
        examples=["evt_01aaaaaaaaaaaaaaaaaaaaaaaa"],
    )
    timestamp: float = Field(
        ...,
        description="Unix timestamp (ms) for when the event was created.",
        examples=[1634567890123],
    )
    session: str = Field(..., examples=["default"])
    metadata: dict[str, Any] | None = Field(
        None,
        description="Metadata for the session.",
        examples=[{"user.id": "123", "user.email": "email@example.com"}],
    )
    engine: Engine = Field(..., examples=["WEBJS"])
    event: dict[str, Any] = Field(
        ...,
        description="The event is triggered when a user edits a previously sent message.",
    )
    payload: WAMessageEditedBody
    me: MeInfo | None = None
    environment: WAHAEnvironment


class GroupInfo(BaseModel):
    id: str = Field(..., examples=["123456789@g.us"])
    subject: str = Field(..., examples=["Group Name"])
    description: str = Field(..., examples=["Group Description"])
    invite: str | None = Field(
        None,
        description="Invite URL",
        examples=["https://chat.whatsapp.com/1234567890abcdef"],
    )
    membersCanAddNewMember: bool = Field(..., description="Members can add new members")
    membersCanSendMessages: bool = Field(..., description="Members can send messages to the group")
    newMembersApprovalRequired: bool = Field(..., description="Admin approval required for new members")
    participants: list[GroupParticipant]


class GroupV2JoinEvent(BaseModel):
    timestamp: float = Field(..., description="Unix timestamp", examples=[1666943582])
    group: GroupInfo
    field_data: dict[str, Any] = Field(..., alias="_data")


class WebhookGroupV2Join(BaseModel):
    id: str = Field(
        ...,
        description="Unique identifier for the event - lower case ULID format. https://github.com/ulid/spec",
        examples=["evt_01aaaaaaaaaaaaaaaaaaaaaaaa"],
    )
    timestamp: float = Field(
        ...,
        description="Unix timestamp (ms) for when the event was created.",
        examples=[1634567890123],
    )
    session: str = Field(..., examples=["default"])
    metadata: dict[str, Any] | None = Field(
        None,
        description="Metadata for the session.",
        examples=[{"user.id": "123", "user.email": "email@example.com"}],
    )
    engine: Engine = Field(..., examples=["WEBJS"])
    event: dict[str, Any] = Field(..., description="When you joined or were added to a group")
    payload: GroupV2JoinEvent
    me: MeInfo | None = None
    environment: WAHAEnvironment


class GroupId(BaseModel):
    id: str = Field(..., examples=["123456789@g.us"])


class GroupV2LeaveEvent(BaseModel):
    timestamp: float = Field(..., description="Unix timestamp", examples=[1666943582])
    group: GroupId
    field_data: dict[str, Any] = Field(..., alias="_data")


class WebhookGroupV2Leave(BaseModel):
    id: str = Field(
        ...,
        description="Unique identifier for the event - lower case ULID format. https://github.com/ulid/spec",
        examples=["evt_01aaaaaaaaaaaaaaaaaaaaaaaa"],
    )
    timestamp: float = Field(
        ...,
        description="Unix timestamp (ms) for when the event was created.",
        examples=[1634567890123],
    )
    session: str = Field(..., examples=["default"])
    metadata: dict[str, Any] | None = Field(
        None,
        description="Metadata for the session.",
        examples=[{"user.id": "123", "user.email": "email@example.com"}],
    )
    engine: Engine = Field(..., examples=["WEBJS"])
    event: dict[str, Any] = Field(..., description="When you left or were removed from a group")
    payload: GroupV2LeaveEvent
    me: MeInfo | None = None
    environment: WAHAEnvironment


class GroupV2UpdateEvent(BaseModel):
    timestamp: float = Field(..., description="Unix timestamp", examples=[1666943582])
    group: dict[str, Any]
    field_data: dict[str, Any] = Field(..., alias="_data")


class WebhookGroupV2Update(BaseModel):
    id: str = Field(
        ...,
        description="Unique identifier for the event - lower case ULID format. https://github.com/ulid/spec",
        examples=["evt_01aaaaaaaaaaaaaaaaaaaaaaaa"],
    )
    timestamp: float = Field(
        ...,
        description="Unix timestamp (ms) for when the event was created.",
        examples=[1634567890123],
    )
    session: str = Field(..., examples=["default"])
    metadata: dict[str, Any] | None = Field(
        None,
        description="Metadata for the session.",
        examples=[{"user.id": "123", "user.email": "email@example.com"}],
    )
    engine: Engine = Field(..., examples=["WEBJS"])
    event: dict[str, Any] = Field(..., description="When group info is updated")
    payload: GroupV2UpdateEvent
    me: MeInfo | None = None
    environment: WAHAEnvironment


class Type1(Enum):
    join = "join"
    leave = "leave"
    promote = "promote"
    demote = "demote"


class GroupV2ParticipantsEvent(BaseModel):
    type: Type1 = Field(..., description="Type of the event")
    timestamp: float = Field(..., description="Unix timestamp", examples=[1666943582])
    group: GroupId
    participants: list[GroupParticipant]
    field_data: dict[str, Any] = Field(..., alias="_data")


class WebhookGroupV2Participants(BaseModel):
    id: str = Field(
        ...,
        description="Unique identifier for the event - lower case ULID format. https://github.com/ulid/spec",
        examples=["evt_01aaaaaaaaaaaaaaaaaaaaaaaa"],
    )
    timestamp: float = Field(
        ...,
        description="Unix timestamp (ms) for when the event was created.",
        examples=[1634567890123],
    )
    session: str = Field(..., examples=["default"])
    metadata: dict[str, Any] | None = Field(
        None,
        description="Metadata for the session.",
        examples=[{"user.id": "123", "user.email": "email@example.com"}],
    )
    engine: Engine = Field(..., examples=["WEBJS"])
    event: dict[str, Any] = Field(..., description="When participants changed - join, leave, promote to admin")
    payload: GroupV2ParticipantsEvent
    me: MeInfo | None = None
    environment: WAHAEnvironment


class WAHAWebhookPresenceUpdate(BaseModel):
    id: str = Field(
        ...,
        description="Unique identifier for the event - lower case ULID format. https://github.com/ulid/spec",
        examples=["evt_01aaaaaaaaaaaaaaaaaaaaaaaa"],
    )
    timestamp: float = Field(
        ...,
        description="Unix timestamp (ms) for when the event was created.",
        examples=[1634567890123],
    )
    session: str = Field(..., examples=["default"])
    metadata: dict[str, Any] | None = Field(
        None,
        description="Metadata for the session.",
        examples=[{"user.id": "123", "user.email": "email@example.com"}],
    )
    engine: Engine = Field(..., examples=["WEBJS"])
    event: dict[str, Any] = Field(..., description="The most recent presence information for a chat.")
    payload: WAHAChatPresences
    me: MeInfo | None = None
    environment: WAHAEnvironment


class PollVote(BaseModel):
    id: str = Field(
        ...,
        description="Message ID",
        examples=["false_11111111111@c.us_AAAAAAAAAAAAAAAAAAAA"],
    )
    selectedOptions: list[str] = Field(..., description="Option that user has selected", examples=[["Awesome!"]])
    timestamp: float = Field(..., description="Timestamp, ms", examples=[1692861369])
    to: str
    from_: str = Field(..., alias="from")
    fromMe: bool
    participant: str | None = None


class MessageDestination(BaseModel):
    id: str = Field(
        ...,
        description="Message ID",
        examples=["false_11111111111@c.us_AAAAAAAAAAAAAAAAAAAA"],
    )
    to: str
    from_: str = Field(..., alias="from")
    fromMe: bool
    participant: str | None = None


class PollVotePayload(BaseModel):
    vote: PollVote
    poll: MessageDestination
    field_data: dict[str, Any] | None = Field(None, alias="_data")


class WAHAWebhookPollVote(BaseModel):
    id: str = Field(
        ...,
        description="Unique identifier for the event - lower case ULID format. https://github.com/ulid/spec",
        examples=["evt_01aaaaaaaaaaaaaaaaaaaaaaaa"],
    )
    timestamp: float = Field(
        ...,
        description="Unix timestamp (ms) for when the event was created.",
        examples=[1634567890123],
    )
    session: str = Field(..., examples=["default"])
    metadata: dict[str, Any] | None = Field(
        None,
        description="Metadata for the session.",
        examples=[{"user.id": "123", "user.email": "email@example.com"}],
    )
    engine: Engine = Field(..., examples=["WEBJS"])
    event: dict[str, Any] = Field(..., description="With this event, you receive new votes for the poll sent.")
    payload: PollVotePayload
    me: MeInfo | None = None
    environment: WAHAEnvironment


class WAHAWebhookPollVoteFailed(BaseModel):
    id: str = Field(
        ...,
        description="Unique identifier for the event - lower case ULID format. https://github.com/ulid/spec",
        examples=["evt_01aaaaaaaaaaaaaaaaaaaaaaaa"],
    )
    timestamp: float = Field(
        ...,
        description="Unix timestamp (ms) for when the event was created.",
        examples=[1634567890123],
    )
    session: str = Field(..., examples=["default"])
    metadata: dict[str, Any] | None = Field(
        None,
        description="Metadata for the session.",
        examples=[{"user.id": "123", "user.email": "email@example.com"}],
    )
    engine: Engine = Field(..., examples=["WEBJS"])
    event: dict[str, Any] = Field(
        ...,
        description="There may be cases when it fails to decrypt a vote from the user. Read more about how to handle such events in the documentations.",
    )
    payload: PollVotePayload
    me: MeInfo | None = None
    environment: WAHAEnvironment


class ChatArchiveEvent(BaseModel):
    id: str = Field(..., examples=["11111111111@c.us"])
    archived: bool
    timestamp: float


class WAHAWebhookChatArchive(BaseModel):
    id: str = Field(
        ...,
        description="Unique identifier for the event - lower case ULID format. https://github.com/ulid/spec",
        examples=["evt_01aaaaaaaaaaaaaaaaaaaaaaaa"],
    )
    timestamp: float = Field(
        ...,
        description="Unix timestamp (ms) for when the event was created.",
        examples=[1634567890123],
    )
    session: str = Field(..., examples=["default"])
    metadata: dict[str, Any] | None = Field(
        None,
        description="Metadata for the session.",
        examples=[{"user.id": "123", "user.email": "email@example.com"}],
    )
    engine: Engine = Field(..., examples=["WEBJS"])
    event: dict[str, Any] = Field(
        ...,
        description="The event is triggered when the chat is archived or unarchived",
    )
    payload: ChatArchiveEvent
    me: MeInfo | None = None
    environment: WAHAEnvironment


class CallData(BaseModel):
    id: str = Field(..., description="Call ID", examples=["ABCDEFGABCDEFGABCDEFGABCDEFG"])
    from_: str | None = Field(None, alias="from", examples=["11111111111@c.us"])
    timestamp: float
    isVideo: bool
    isGroup: bool
    field_data: dict[str, Any] = Field(..., alias="_data")


class WAHAWebhookCallReceived(BaseModel):
    id: str = Field(
        ...,
        description="Unique identifier for the event - lower case ULID format. https://github.com/ulid/spec",
        examples=["evt_01aaaaaaaaaaaaaaaaaaaaaaaa"],
    )
    timestamp: float = Field(
        ...,
        description="Unix timestamp (ms) for when the event was created.",
        examples=[1634567890123],
    )
    session: str = Field(..., examples=["default"])
    metadata: dict[str, Any] | None = Field(
        None,
        description="Metadata for the session.",
        examples=[{"user.id": "123", "user.email": "email@example.com"}],
    )
    engine: Engine = Field(..., examples=["WEBJS"])
    event: dict[str, Any] = Field(..., description="The event is triggered when the call is received by the user.")
    payload: CallData
    me: MeInfo | None = None
    environment: WAHAEnvironment


class WAHAWebhookCallAccepted(BaseModel):
    id: str = Field(
        ...,
        description="Unique identifier for the event - lower case ULID format. https://github.com/ulid/spec",
        examples=["evt_01aaaaaaaaaaaaaaaaaaaaaaaa"],
    )
    timestamp: float = Field(
        ...,
        description="Unix timestamp (ms) for when the event was created.",
        examples=[1634567890123],
    )
    session: str = Field(..., examples=["default"])
    metadata: dict[str, Any] | None = Field(
        None,
        description="Metadata for the session.",
        examples=[{"user.id": "123", "user.email": "email@example.com"}],
    )
    engine: Engine = Field(..., examples=["WEBJS"])
    event: dict[str, Any] = Field(..., description="The event is triggered when the call is accepted by the user.")
    payload: CallData
    me: MeInfo | None = None
    environment: WAHAEnvironment


class WAHAWebhookCallRejected(BaseModel):
    id: str = Field(
        ...,
        description="Unique identifier for the event - lower case ULID format. https://github.com/ulid/spec",
        examples=["evt_01aaaaaaaaaaaaaaaaaaaaaaaa"],
    )
    timestamp: float = Field(
        ...,
        description="Unix timestamp (ms) for when the event was created.",
        examples=[1634567890123],
    )
    session: str = Field(..., examples=["default"])
    metadata: dict[str, Any] | None = Field(
        None,
        description="Metadata for the session.",
        examples=[{"user.id": "123", "user.email": "email@example.com"}],
    )
    engine: Engine = Field(..., examples=["WEBJS"])
    event: dict[str, Any] = Field(..., description="The event is triggered when the call is rejected by the user.")
    payload: CallData
    me: MeInfo | None = None
    environment: WAHAEnvironment


class WAHAWebhookLabelUpsert(BaseModel):
    id: str = Field(
        ...,
        description="Unique identifier for the event - lower case ULID format. https://github.com/ulid/spec",
        examples=["evt_01aaaaaaaaaaaaaaaaaaaaaaaa"],
    )
    timestamp: float = Field(
        ...,
        description="Unix timestamp (ms) for when the event was created.",
        examples=[1634567890123],
    )
    session: str = Field(..., examples=["default"])
    metadata: dict[str, Any] | None = Field(
        None,
        description="Metadata for the session.",
        examples=[{"user.id": "123", "user.email": "email@example.com"}],
    )
    engine: Engine = Field(..., examples=["WEBJS"])
    event: dict[str, Any] = Field(..., description="The event is triggered when a label is created or updated")
    payload: Label
    me: MeInfo | None = None
    environment: WAHAEnvironment


class WAHAWebhookLabelDeleted(BaseModel):
    id: str = Field(
        ...,
        description="Unique identifier for the event - lower case ULID format. https://github.com/ulid/spec",
        examples=["evt_01aaaaaaaaaaaaaaaaaaaaaaaa"],
    )
    timestamp: float = Field(
        ...,
        description="Unix timestamp (ms) for when the event was created.",
        examples=[1634567890123],
    )
    session: str = Field(..., examples=["default"])
    metadata: dict[str, Any] | None = Field(
        None,
        description="Metadata for the session.",
        examples=[{"user.id": "123", "user.email": "email@example.com"}],
    )
    engine: Engine = Field(..., examples=["WEBJS"])
    event: dict[str, Any] = Field(..., description="The event is triggered when a label is deleted")
    payload: Label
    me: MeInfo | None = None
    environment: WAHAEnvironment


class LabelChatAssociation(BaseModel):
    labelId: str = Field(..., description="Label ID", examples=["1"])
    chatId: str = Field(..., description="Chat ID", examples=["11111111111@c.us"])
    label: Label


class WAHAWebhookLabelChatAdded(BaseModel):
    id: str = Field(
        ...,
        description="Unique identifier for the event - lower case ULID format. https://github.com/ulid/spec",
        examples=["evt_01aaaaaaaaaaaaaaaaaaaaaaaa"],
    )
    timestamp: float = Field(
        ...,
        description="Unix timestamp (ms) for when the event was created.",
        examples=[1634567890123],
    )
    session: str = Field(..., examples=["default"])
    metadata: dict[str, Any] | None = Field(
        None,
        description="Metadata for the session.",
        examples=[{"user.id": "123", "user.email": "email@example.com"}],
    )
    engine: Engine = Field(..., examples=["WEBJS"])
    event: dict[str, Any] = Field(..., description="The event is triggered when a label is added to a chat")
    payload: LabelChatAssociation
    me: MeInfo | None = None
    environment: WAHAEnvironment


class WAHAWebhookLabelChatDeleted(BaseModel):
    id: str = Field(
        ...,
        description="Unique identifier for the event - lower case ULID format. https://github.com/ulid/spec",
        examples=["evt_01aaaaaaaaaaaaaaaaaaaaaaaa"],
    )
    timestamp: float = Field(
        ...,
        description="Unix timestamp (ms) for when the event was created.",
        examples=[1634567890123],
    )
    session: str = Field(..., examples=["default"])
    metadata: dict[str, Any] | None = Field(
        None,
        description="Metadata for the session.",
        examples=[{"user.id": "123", "user.email": "email@example.com"}],
    )
    engine: Engine = Field(..., examples=["WEBJS"])
    event: dict[str, Any] = Field(..., description="The event is triggered when a label is deleted from a chat")
    payload: LabelChatAssociation
    me: MeInfo | None = None
    environment: WAHAEnvironment


class Response(Enum):
    UNKNOWN = "UNKNOWN"
    GOING = "GOING"
    NOT_GOING = "NOT_GOING"
    MAYBE = "MAYBE"


class EventResponse(BaseModel):
    response: Response
    timestampMs: float
    extraGuestCount: float


class EventResponsePayload(BaseModel):
    id: str = Field(
        ...,
        description="Message ID",
        examples=["false_11111111111@c.us_AAAAAAAAAAAAAAAAAAAA"],
    )
    timestamp: float = Field(
        ...,
        description="Unix timestamp for when the message was created",
        examples=[1666943582],
    )
    from_: str = Field(
        ...,
        alias="from",
        description="ID for the Chat that this message was sent to, except if the message was sent by the current user ",
        examples=["11111111111@c.us"],
    )
    fromMe: bool = Field(..., description="Indicates if the message was sent by the current user")
    source: Source = Field(
        ...,
        description='The device that sent the message - either API or APP. Available in events (webhooks/websockets) only and only "fromMe: true" messages.',
        examples=["api"],
    )
    to: str = Field(
        ...,
        description="\n* ID for who this message is for.\n* If the message is sent by the current user, it will be the Chat to which the message is being sent.\n* If the message is sent by another user, it will be the ID for the current user.\n",
        examples=["11111111111@c.us"],
    )
    participant: str = Field(..., description="For groups - participant who sent the message")
    field_data: dict[str, Any] | None = Field(
        None,
        alias="_data",
        description="Message in a raw format that we get from WhatsApp. May be changed anytime, use it with caution! It depends a lot on the underlying backend.",
    )
    eventCreationKey: MessageDestination
    eventResponse: EventResponse | None = None


class WAHAWebhookEventResponse(BaseModel):
    id: str = Field(
        ...,
        description="Unique identifier for the event - lower case ULID format. https://github.com/ulid/spec",
        examples=["evt_01aaaaaaaaaaaaaaaaaaaaaaaa"],
    )
    timestamp: float = Field(
        ...,
        description="Unix timestamp (ms) for when the event was created.",
        examples=[1634567890123],
    )
    session: str = Field(..., examples=["default"])
    metadata: dict[str, Any] | None = Field(
        None,
        description="Metadata for the session.",
        examples=[{"user.id": "123", "user.email": "email@example.com"}],
    )
    engine: Engine = Field(..., examples=["WEBJS"])
    event: dict[str, Any] = Field(..., description="The event is triggered when the event response is received.")
    payload: EventResponsePayload
    me: MeInfo | None = None
    environment: WAHAEnvironment


class WAHAWebhookEventResponseFailed(BaseModel):
    id: str = Field(
        ...,
        description="Unique identifier for the event - lower case ULID format. https://github.com/ulid/spec",
        examples=["evt_01aaaaaaaaaaaaaaaaaaaaaaaa"],
    )
    timestamp: float = Field(
        ...,
        description="Unix timestamp (ms) for when the event was created.",
        examples=[1634567890123],
    )
    session: str = Field(..., examples=["default"])
    metadata: dict[str, Any] | None = Field(
        None,
        description="Metadata for the session.",
        examples=[{"user.id": "123", "user.email": "email@example.com"}],
    )
    engine: Engine = Field(..., examples=["WEBJS"])
    event: dict[str, Any] = Field(
        ...,
        description="The event is triggered when the event response is failed to decrypt.",
    )
    payload: EventResponsePayload
    me: MeInfo | None = None
    environment: WAHAEnvironment


class EnginePayload(BaseModel):
    event: str
    data: dict[str, Any]


class WAHAWebhookEngineEvent(BaseModel):
    id: str = Field(
        ...,
        description="Unique identifier for the event - lower case ULID format. https://github.com/ulid/spec",
        examples=["evt_01aaaaaaaaaaaaaaaaaaaaaaaa"],
    )
    timestamp: float = Field(
        ...,
        description="Unix timestamp (ms) for when the event was created.",
        examples=[1634567890123],
    )
    session: str = Field(..., examples=["default"])
    metadata: dict[str, Any] | None = Field(
        None,
        description="Metadata for the session.",
        examples=[{"user.id": "123", "user.email": "email@example.com"}],
    )
    engine: Engine = Field(..., examples=["WEBJS"])
    event: dict[str, Any] = Field(..., description="Internal engine event.")
    payload: EnginePayload
    me: MeInfo | None = None
    environment: WAHAEnvironment


class WAHAWebhookGroupJoin(BaseModel):
    id: str = Field(
        ...,
        description="Unique identifier for the event - lower case ULID format. https://github.com/ulid/spec",
        examples=["evt_01aaaaaaaaaaaaaaaaaaaaaaaa"],
    )
    timestamp: float = Field(
        ...,
        description="Unix timestamp (ms) for when the event was created.",
        examples=[1634567890123],
    )
    session: str = Field(..., examples=["default"])
    metadata: dict[str, Any] | None = Field(
        None,
        description="Metadata for the session.",
        examples=[{"user.id": "123", "user.email": "email@example.com"}],
    )
    engine: Engine = Field(..., examples=["WEBJS"])
    event: dict[str, Any] = Field(..., deprecated=True, description="Some one join a group.")
    payload: dict[str, Any]
    me: MeInfo | None = None
    environment: WAHAEnvironment


class WAHAWebhookGroupLeave(BaseModel):
    id: str = Field(
        ...,
        description="Unique identifier for the event - lower case ULID format. https://github.com/ulid/spec",
        examples=["evt_01aaaaaaaaaaaaaaaaaaaaaaaa"],
    )
    timestamp: float = Field(
        ...,
        description="Unix timestamp (ms) for when the event was created.",
        examples=[1634567890123],
    )
    session: str = Field(..., examples=["default"])
    metadata: dict[str, Any] | None = Field(
        None,
        description="Metadata for the session.",
        examples=[{"user.id": "123", "user.email": "email@example.com"}],
    )
    engine: Engine = Field(..., examples=["WEBJS"])
    event: dict[str, Any] = Field(..., deprecated=True, description="Some one left a group.")
    payload: dict[str, Any]
    me: MeInfo | None = None
    environment: WAHAEnvironment


class WAHAWebhookStateChange(BaseModel):
    id: str = Field(
        ...,
        description="Unique identifier for the event - lower case ULID format. https://github.com/ulid/spec",
        examples=["evt_01aaaaaaaaaaaaaaaaaaaaaaaa"],
    )
    timestamp: float = Field(
        ...,
        description="Unix timestamp (ms) for when the event was created.",
        examples=[1634567890123],
    )
    session: str = Field(..., examples=["default"])
    metadata: dict[str, Any] | None = Field(
        None,
        description="Metadata for the session.",
        examples=[{"user.id": "123", "user.email": "email@example.com"}],
    )
    engine: Engine = Field(..., examples=["WEBJS"])
    event: dict[str, Any] = Field(
        ...,
        deprecated=True,
        description="It’s an internal engine’s state, not session status.",
    )
    payload: dict[str, Any]
    me: MeInfo | None = None
    environment: WAHAEnvironment
